...
# Claude Code v2.1.34 - Hotfix

Release menor, solo fixes de estabilidad y seguridad:

### Crash agent teams
- Fix: crash al cambiar la setting de agent teams entre renders

### Bypass de permisos Bash (seguridad)
- **Bug**: Commands excluidos del sandbox (`sandbox.excludedCommands` o `dangerouslyDisableSandbox`) podían saltarse la regla de `ask` permission cuando `autoAllowBashIfSandboxed` estaba activo
- **Riesgo**: Un command peligroso podía ejecutarse sin pedir confirmación al usuario
- **Fix**: Ahora respeta el `ask` permission independientemente de la configuración de sandbox
